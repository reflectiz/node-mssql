...
class PreparedStatement extends EventEmitter {
  ...
  acquire (request, callback) {
    ...
    if (this._activeRequest) {
      callback(new TransactionError("Can't acquire connection for the request. There is another request in progress.", 'EREQINPROG'))
      return this
    }
    ...
    callback(null, this._acquiredConnection, this._acquiredConfig)
    return this
  }
  ...
  _prepare (statement, callback) {
    ...
    if (this.prepared) {
      return callback(new PreparedStatementError('Statement is already prepared.', 'EALREADYPREPARED'))
    }
  ...
  _unprepare (callback) {
    ...
    if (!this.prepared) {
      return callback(new PreparedStatementError('Statement is not prepared. Call prepare() first.', 'ENOTPREPARED'))
    }

    if (this._activeRequest) {
      return callback(new TransactionError("Can't unprepare the statement. There is a request in progress.", 'EREQINPROG'))
    }
...
class Transaction extends EventEmitter {
  ...
  _begin (isolationLevel, callback) {
    ...
    if (this._acquiredConnection) {
      return callback(new TransactionError('Transaction has already begun.', 'EALREADYBEGUN'))
    }
  ...
  _commit (callback) {
    ...
    if (this._aborted) {
      return callback(new TransactionError('Transaction has been aborted.', 'EABORT'))
    }

    if (!this._acquiredConnection) {
      return callback(new TransactionError('Transaction has not begun. Call begin() first.', 'ENOTBEGUN'))
    }

    if (this._activeRequest) {
      return callback(new TransactionError("Can't commit transaction. There is a request in progress.", 'EREQINPROG'))
    }
  ...
  _rollback (callback) {
    ...
    if (this._activeRequest) {
      return callback(new TransactionError("Can't rollback transaction. There is a request in progress.", 'EREQINPROG'))
    }
...
class Request extends EventEmitter {
  ...
  _batch (batch, callback) {
    if (!this.connection) {
      return callback(new RequestError('No connection is specified for that request.', 'ENOCONN'))
    }

    if (!this.connection.connected) {
      return callback(new ConnectionError('Connection is closed.', 'ECONNCLOSED'))
    }
